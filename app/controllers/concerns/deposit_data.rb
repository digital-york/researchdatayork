...
module DepositData
  ...
  def validate_deposit_chunk (filechunk, target_file, upload_dir, dataset_id, write_mode)
    ...
    elsif !(current_user and current_user.email and (current_user.admin? or (dataset.creator_string and dataset.creator_string.include? current_user.email.split("@")[0])))
